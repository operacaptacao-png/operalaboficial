...
export default function LoginScreen({ onLogin }: LoginScreenProps) {
  ...
  const handleStudentSubmit = (e: React.FormEvent) => {
    ...
    const checkMatchPass = (turma: string, pass: string) => {
      const cTurma = cleanCode(turma);
      const cPass = cleanCode(pass);
      if (cTurma === cPass) return true;
      // Compatibilidade entre prefixos TARGETON e TGT1ON
      const normT = cTurma.replace('TARGETON', 'TGT1ON');
      const normP = cPass.replace('TARGETON', 'TGT1ON');
      if (normT === normP) return true;
      // Compatibilidade entre finais de semestre 261 e 262 para turmas online
      if (normT.replace(/26[12]$/, '') === normP.replace(/26[12]$/, '')) return true;
      return false;
    };

    const validStudent = matches.find(s => checkMatchPass(s.turma, typedPass));
