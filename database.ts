...
export const PERF_MASTER_DB: Record<string, Record<string, string[]>> = {
  ...
  "EDIMO": {
    ...
    "TARGETONSAB08A26.1": ["Sophia e Souza Alves", "Thalita Oliveira Suzarte", "Matheus Cardoso Santos", "Anna Leticia Marinho Souza", "Lorena Simas Daltro"],
    "TARGETONQUI19A26.1": ["Ana Carla Cerqueira De Jesus", "Carlos Eduardo Cerqueira de Jesus", "Carlos Henrique Cerqueira de Jesus", "Laila Santos Pinheiro Guerra"]
  },
  "IANNE": {
...
export const HORARIOS_ATIVOS: Record<string, Record<number, { s: string; e: string; mod?: string }[]>> = {
  ...
  "EDIMO": {
    ...
    4: [
      { s: "14:00", e: "16:00" },
      { s: "19:00", e: "20:00", mod: "Online" }
    ], // Quinta
    6: [
      { s: "08:00", e: "10:00", mod: "Online" }
    ]  // Sábado
  },
  "JOÃO": {
...
export function mapearTurmaParaCertificado(turmaCodigo: string): CertificateConfig | null {
  ...
  if (prefixo.startsWith('TGT1') || prefixo.startsWith('TARGETON') || prefixo.startsWith('TARGET1')) return { idioma: 'en', curso: 'General English Course', horas: 90, nivel: 'A1/A2' };
  if (prefixo.startsWith('TGT2') || prefixo.startsWith('TARGET2')) return { idioma: 'en', curso: 'General English Course', horas: 90, nivel: 'A2/B1' };
  if (prefixo.startsWith('TGT3') || prefixo.startsWith('TARGET3')) return { idioma: 'en', curso: 'General English Course', horas: 90, nivel: 'B1/B2' };
